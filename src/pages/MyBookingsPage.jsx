// In src/pages/MyBookingsPage.jsx
...
function BookingCard({ booking }) {
  // Use the unit's first image or a placeholder
  const imageUrl = booking.unit.images?.[0]?.image || `https://via.placeholder.com/300x200.png?text=${encodeURIComponent(booking.unit.unit_name_or_number)}`;
        ...
        <div className="mt-4 space-y-2">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p className="text-gray-800">{new Date(booking.check_in).toLocaleDateString('en-US', { 
                year: 'numeric', 
                month: 'short', 
                day: 'numeric' 
              })}</p>
            ...
            <div>
              ...
              <p className="text-gray-800">{new Date(booking.check_out).toLocaleDateString('en-US', { 
                year: 'numeric', 
                month: 'short', 
                day: 'numeric' 
              })}</p>
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      if (user) {
        setLoading(true);
        try {
          // Try the my-bookings endpoint first, fallback to regular bookings endpoint
